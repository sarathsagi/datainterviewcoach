...
export default async function PythonProblemPage({ params }: PageProps) {
  ...
  // Fetch this question + all Python questions for prev/next navigation
  const [question, allQuestions] = await Promise.all([
    prisma.practiceQuestion.findUnique({
      where: { slug, isPublished: true, category: "PYTHON" },
    }),
      ...
      {question.context && (
        <div className="rounded-lg border border-slate-800 bg-slate-950/60 overflow-hidden">
          <div className="px-4 py-2 border-b border-slate-800 bg-slate-900/50">
            <span className="text-xs font-medium text-slate-400">Setup / Given Data</span>
          </div>
          <div className="p-4">
